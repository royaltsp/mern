...
module.exports = (app) => {
  ...
  app.post('/transaction/broadcast', function (req, res) {
    ...
    const new_transaction = Securum.createNewTransaction(req.body.amount, req.body.sender, req.body.recipient);
    ...
    Promise.all(reg_nodes_promises)
      .then(data => {
        const bulk_register_options = {
          ...
          method: 'POST',
          body: { all_network_nodes: [...Securum.network_nodes, Securum.current_node_url] },
          json: true
        };
  ...
  app.post('/register-node', function (req, res) {
    const new_node_url = req.body.new_node_url;
    const node_not_already_present = Securum.network_nodes.indexOf(new_node_url) == -1;
  ...
  app.post('/register-nodes-bulk', function (req, res) {
    ...
    all_network_nodes.forEach(network_node_url => {
      const node_not_already_present = Securum.network_nodes.indexOf(network_node_url) == -1;
  ...
  app.get('/consensus', function (req, res) {
    ...
    Promise.all(requestPromises)
      .then(blockchains => {
        ...
        if (!newLongestChain || (newLongestChain && !Securum.chainIsValid(newLongestChain))) {
          res.json({
            note: 'Current chain has not been replaced.',
            chain: Securum.chain
          });
